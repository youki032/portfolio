...
private static final long serialVersionUID = 1L;
  ...
  private void makePaging() {
    ...
    if(this.cPageNo > finalPage) {
      this.setcPageNo(finalPage); //기본값 설정
    }
      
    if(this.cPageNo < 0 || this.cPageNo > finalPage) {
      this.cPageNo = 1;  //현재 페이지 유효성 체크
    }
    
    boolean isNowFirst = cPageNo == 1? true : false;  //시작 페이지(전체)
    
    boolean isNowFinal = cPageNo == finalPage ? true : false; //마지막 페이지(전체)
  
    int startPage = ((cPageNo -1)/ 10) * 10 + 1; //시작 페이지(페이징 네비기준)
    ...
    if(isNowFirst) {
      ...
      this.setPrevPageNo(((cPageNo - 1) < 1? 1 : (cPageNo - 1))); //이전 페이지 번호
    }
    ...
    if(isNowFinal) {
      ...
      this.setNextPageNo(((cPageNo + 1) > finalPage? finalPage : (cPageNo + 1))); 
      //다음 페이지 번호
    }
